...
Verifies all 10 JSON result files in benchmarks/results/{vllm,sglang}/ to ensure:
...
3. Engine references match v0.25.1 (vLLM) and v0.5.12-cu130 (SGLang).
...
def main():
    ...
    if not root.exists():
        print(f"ERROR: Results directory {root} does not exist!", file=sys.stderr)
        sys.exit(1)

    engines = {
        "vllm": {"expected_ver_norm": "0.25.1", "expected_img_sub": "vllm-blackwell", "expected_ver_raw_options": ["v0.25.1", "0.25.1"]},
        "sglang": {"expected_ver_norm": "0.5.12", "expected_img_sub": "sglang-blackwell", "expected_ver_raw_options": ["v0.5.12-cu130", "0.5.12-cu130", "v0.5.12", "0.5.12"]}
    }
    suites = ["standard", "massive", "soak", "saturation", "prefill"]

    print("==============================================================================")
    print("GLM-5.2 GCP Enterprise Serving - Benchmark Data Integrity Stress-Test")
    print("==============================================================================")
    ...
    for eng, rules in engines.items():
        ...
        for s in suites:
            ...
            if norm_ver != rules["expected_ver_norm"]:
                errors.append(f"[{eng}/{s}] Version mismatch: found '{ver}' (norm '{norm_ver}'), expected norm '{rules['expected_ver_norm']}'")
            ...
            if s in ["standard", "massive", "soak"]:
                ...
                print(f"    [OK] {s:<10} | ver={ver:<14} | ts={ts_str:<22} | levels={[item['concurrency'] for item in sweep]} | max_tps={max(item['aggregate_tok_s'] for item in sweep):6.2f}")
