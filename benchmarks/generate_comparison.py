...
def generate_markdown(vllm: dict, sglang: dict) -> str:
  ...
  lines.append("")
  lines.append(
      "All benchmarks were executed on the live GKE serving cluster with"
      " identical hardware allocations (8x NVIDIA B200 HGX, GKE `a4-highgpu-8g`"
      " node pool, NVLink 5th-gen, tensor parallelism TP=8) and identical model"
      " weights (`nvidia/GLM-5.2-NVFP4`) mounted read-only from a shared"
      " Hyperdisk ML ROX volume. Both engines served via the LiteLLM Enterprise"
      " Gateway on port 4000 (Standard, Massive, Soak) and direct container"
      " port 8000 (Saturation Sweep, Prefill Ingestion)."
  )
  lines.append("")
  lines.append("#### Methodology & Provenance Protocol")
  lines.append(
      "* **Cache Policy:** Workload suites (Standard, Massive, Soak) evaluated"
      " end-to-end serving performance on port 4000, where dynamic prompt nonce"
      " injection bypassed LiteLLM Redis exact-match caching. The Concurrency"
      " Saturation Sweep and Prefill Ingestion suites evaluated direct engine"
      " performance on port 8000, utilizing unique prompt sets and radix cache"
      " flushing to ensure 0% prefix-cache hits (measuring true cold decoding"
      " and prefill throughput)."
  )
  lines.append(
      "* **Sequential Execution & Drain Protocol:** To prevent resource"
      " contention and queue contamination, benchmark suites were executed"
      " strictly sequentially with full queue drain intervals between runs."
  )
  lines.append(
      "* **Engine Provenance Verification:** Engine identity and container"
      " provenance were verified prior to every suite by inspecting `/metrics`"
      " endpoints (`^vllm:` vs `^sglang:`) and deployment container images."
      " Collection timestamps recorded in suite metadata:"
  )
  ...
  v_ts_list = [
      f"{s.capitalize()} ({get_metadata(vllm[s]).get('run_timestamp', 'N/A')})"
      for s in ["standard", "massive", "soak", "saturation", "prefill"]
  ]
  g_ts_list = [
      f"{s.capitalize()}"
      f" ({get_metadata(sglang[s]).get('run_timestamp', 'N/A')})"
      for s in ["standard", "massive", "soak", "saturation", "prefill"]
  ]
  ...
  lines.append(
      f"| Workload Suite | Metric | {v_label} | {g_label} | Delta ($\\Delta$) |"
  )
  lines.append("| :--- | :--- | :--- | :--- | :--- |")

  suites_t1 = [
      (
          "Standard Suite ($c=8$, $128\\text{ tok}$)",
          "standard",
          [
              (
                  "TTFT P50 (ms)",
                  lambda d: d["metrics"]["ttft_ms"]["p50"],
                  False,
                  "{:.2f}",
              ),
              (
                  "TPOT mean (ms)",
                  lambda d: d["metrics"]["tpot_ms"]["mean"],
                  False,
                  "{:.2f}",
              ),
              (
                  "Throughput (tok/s)",
                  lambda d: d["throughput_tokens_sec"],
                  True,
                  "{:.2f}",
              ),
              (
      ...
      (
          "Massive Stress ($c=20$, $256\\text{ tok}$)",
          "massive",
          [
              (
                  "TTFT P50 (ms)",
      ...
      (
          "Endurance Soak ($c=18$, $1800\\text{s}$)",
          "soak",
          [
              (
                  "TTFT P50 (ms)",
              ...
              (
                  "Completed cycles",
                  lambda d: float(d["successful_requests"]),
                  True,
                  "{:.0f}",
              ),
          ],
